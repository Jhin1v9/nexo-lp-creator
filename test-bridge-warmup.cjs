...
(async () => {
  const context = BridgeAdapter.initializeContext('debug-session-warmup');
  try {
    const response = await BridgeAdapter.sendMessage(context, 'site de pao de queijo artesanal', { stack: 'static-html-tailwind', phase: 'code' });
