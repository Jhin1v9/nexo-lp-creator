...
<template>
  <div class="min-h-screen">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <div class="hidden md:flex items-center space-x-8">
            <a v-for="link in navLinks" :key="link.href" :href="link.href" class="text-sm text-slate-600 hover:text-slate-900 transition-colors">
              {{ link.label }}
            </a>
            <a href="#cta" class="text-sm font-medium text-white bg-brand-600 hover:bg-brand-700 px-4 py-2 rounded-lg transition-colors">
              Get Started
            </a>
          </div>
          <button class="md:hidden p-2 rounded-lg text-slate-600 hover:bg-slate-100" @click="mobileMenuOpen = !mobileMenuOpen" aria-label="Toggle menu">
        ...
        <div v-if="mobileMenuOpen" class="md:hidden pb-4 space-y-2">
          <a v-for="link in navLinks" :key="link.href" :href="link.href" class="block px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 rounded-lg">
            {{ link.label }}
          </a>
        </div>
      </div>
    ...
    <header class="relative pt-32 pb-20 lg:pt-40 lg:pb-28 overflow-hidden">
      <div class="absolute inset-0 bg-gradient-to-br from-slate-50 via-white to-brand-50 -z-10" />
      <div class="absolute top-20 right-0 w-96 h-96 bg-brand-200/20 rounded-full blur-3xl -z-10" />
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="max-w-2xl mx-auto text-lg sm:text-xl text-slate-600 mb-10">{{ tagline }}</p>
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a href="#cta" class="w-full sm:w-auto px-8 py-4 text-lg font-semibold text-white bg-brand-600 hover:bg-brand-700 rounded-xl transition-all hover:shadow-lg hover:-translate-y-0.5">
    ...
    <section id="features" class="py-20 lg:py-28 bg-white">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <div v-for="(f, i) in features" :key="i" class="group p-8 rounded-2xl bg-slate-50 hover:bg-white border border-slate-100 hover:border-slate-200 transition-all hover:shadow-lg hover:-translate-y-1">
    ...
    <section id="cta" class="py-20 lg:py-28 relative overflow-hidden">
      <div class="absolute inset-0 bg-gradient-to-br from-brand-600 to-brand-800 -z-10" />
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-6">{{CTA_HEADLINE:Ready to Get Started?}}</h2>
        <a href="#" class="inline-block px-8 py-4 text-lg font-semibold text-brand-700 bg-white hover:bg-brand-50 rounded-xl transition-all hover:shadow-xl hover:-translate-y-0.5">
